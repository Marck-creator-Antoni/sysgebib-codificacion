...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const  buttons = [{
                ...
                text: '<button class="btn btn-success btn-sm" style="margin-right: 8px;"><i class="fa fa-file-excel-o"></i></button>'
            },
            //Botón para PDF
            {
                extend: 'pdf',
                ...
                title: 'Archivo PDF',
                filename: 'reporte',
                text: '<button class="btn btn-secondary btn-sm" style="margin-right: 8px;"><i class="fa fa-file-pdf-o"></i></button>'
            },
            //Botón para print
            {
                extend: 'print',
                ...
                text: '<button class="btn btn-info btn-sm" style="margin-right: 8px;"><i class="fa fa-print"></i></button>'
            },
            {
              ...
              footer: true,
              title: 'Copiar Datos',
              filename: 'Copied_Data',
              text: '<button class="btn btn-secondary btn-sm" style="margin-right: 8px;"><i class="fa fa-copy"></i></button>'
             }
        ]
...
const buttonElements = buttons.map(btn => {
  const button = $(btn.text).attr('data-table', btn.table); // 'btn.table' debe ser el nombre de la tabla
  return button.prop('outerHTML');
});

// Unir los botones generados en un string HTML y añadirlos al contenedor
$('#botonesContainer').html(buttonElements.join(''));

$('#botonesContainer button').on('click', function () {
   const index = $(this).index();

     const tablas = [ tblUsuarios, tblEst, tblMateria,tblAutor,tblEditorial,tblLibros,tblPrestar];

     var tablecont;
     for (const tabla of tablas) {
       if(tabla.context.length>=1){
        tablecont=tabla;
         break;
       }else {   
       }
     }
     tablecont.button(index).trigger();  
});
    ...
    if (document.getElementById('nombre_estudiante')) {
        ...
        http.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                let numeroDeNotificaciones= res.length;

               
// Obtiene el elemento <span>
var notificationCountSpan = document.getElementById("notificationCount");

// Verifica si el número de notificaciones es mayor que 0
if (numeroDeNotificaciones > 0) {
    // Llena el contenido del <span> con el número de notificaciones
    notificationCountSpan.textContent = numeroDeNotificaciones;
} else {
    // Si no hay notificaciones, oculta el <span>
...
for (let i = 0; i < data.length; i++) {
    ...
    cantidades.push(parseInt(data[i]['cantidad_sum']));
}
...
if (document.getElementById("repoplibrosPendientes")) {
    const url = base_url + "Configuracion/grafico_pendientes";
    const http = new XMLHttpRequest();
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
...
var ctx = document.getElementById("repoplibrosPendientes");
var myBarChart = new Chart(ctx, {
    type: 'bar',
    data: {
        labels: fechas,
